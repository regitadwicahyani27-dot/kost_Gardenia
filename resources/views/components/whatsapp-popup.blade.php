{{-- Panggil di halaman manapun: @include('components.whatsapp-popup') --}}
{{-- Tombol floating WhatsApp --}}
<div id="wa-fab" onclick="bukaPopupWA()"
     class="fixed bottom-6 right-6 z-[90] w-14 h-14 rounded-full bg-green-500 text-white flex items-center justify-center shadow-lg cursor-pointer hover:bg-green-600 hover:scale-110 transition-all duration-200">
...
<div id="popup-wa" class="hidden fixed inset-0 z-[998] flex items-center justify-center px-4">
    <div class="absolute inset-0 bg-black/50" style="animation: fadeIn 0.22s ease forwards" onclick="tutupPopupWA()"></div>
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 z-10 text-center" style="animation: popupIn 0.28s cubic-bezier(0.34, 1.46, 0.64, 1) forwards">
