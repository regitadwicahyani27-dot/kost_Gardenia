...
@section('title', 'Tambah Testimoni - Admin Kos Putri Gardenia')
...
<div class="max-w-xl">
    ...
    <h1 class="font-display text-2xl font-bold text-gray-900 mb-1">Tambah Testimoni</h1>
    <p class="text-sm text-gray-500 mb-6">Testimoni ini akan langsung tampil di halaman beranda.</p>

    <form action="{{ route('admin.testimonial.store') }}" method="POST" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        @csrf

        <div>
            <label for="name" class="block text-sm font-semibold text-gray-700 mb-1">Nama Penghuni <span class="text-red-500">*</span></label>
            <input id="name" name="name" type="text" value="{{ old('name') }}" placeholder="Contoh: Eca"
                   class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#2F4538] @error('name') border-red-400 @enderror" />
            @error('name') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label for="label" class="block text-sm font-semibold text-gray-700 mb-1">Label</label>
            <input id="label" name="label" type="text" value="{{ old('label') }}" placeholder="Contoh: Penghuni Aktif, Alumni"
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="4" placeholder="Tuliskan testimoni penghuni di sini..."
                      class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#2F4538] resize-none @error('content') border-red-400 @enderror">{{ old('content') }}</textarea>
